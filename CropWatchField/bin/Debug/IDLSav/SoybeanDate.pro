  Pro SoybeanDate,CCDImageInputpath=CCDImageInputpath,IRSImageInputpath=IRSImageInputpath,Soybeanoutpath=Soybeanoutpath,Message=Message
  compile_opt idl2
  ;距离成熟期越近日期的影像计算结果越好
  ;读取影像数据
  ...
  ;IRS数据的band1和band2
   ...
   SoybeanMD=17.11-4.33*NDVI+37.34*NDWI
   index5=where(SoybeanMD EQ 17.11)
   SoybeanMD[index5]=0
   SoybeanMD=long(SoybeanMD*10)
   SoybeanMD=FLOAT(SoybeanMD)/10
;   write_tiff,Soybeanoutpath+'\'+'距离大豆'+Starttime+'成熟期天数预测.tif',SoybeanMD,geotiff = geotiff,/float
   write_tiff,Soybeanoutpath,SoybeanMD,geotiff = geotiff,/float
   
   Message=''
   end
  
  pro Test_SoybeanDate
  ...
  SoybeanDate,CCDImageInputpath=CCDImageInputpath,IRSImageInputpath=IRSImageInputpath,Soybeanoutpath=Soybeanoutpath,Message=Message
  end