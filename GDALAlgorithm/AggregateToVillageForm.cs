...
{
    public partial class AggregateToVillageForm : Form
    {
        public AggregateToVillageForm()
        {
            InitializeComponent();
        }
        ...
        private void AggregateToVillageForm_Load(object sender, EventArgs e)
        {
            //绑定-----CROPINFO表中DELFLAG=1的，DELFLAG=0为备用值
            //SELECT [CropCode],[CropName]FROM [HXCROPWATCH].[dbo].[CROPINFO] where DELFLAG='1'
            //Dictionary<string, string> dict = DataBaseOperate.setCropSource();
            //BindingSource bs = new BindingSource();
            //bs.DataSource = dict;
            //cbx_croptype.DataSource = bs;
            //cbx_croptype.DisplayMember = "Value";
            //cbx_croptype.ValueMember = "Key";

        }
        //用来展示结果的datatable,最终绑定到datagridview上
        DataTable dt = AggregateToVillage.CreateDatatable();
